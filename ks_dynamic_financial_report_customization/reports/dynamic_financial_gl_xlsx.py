...
class KsDynamicFinancialXlsxGL(models.Model):
    ...
    def ks_get_xlsx_general_ledger(self, ks_df_informations):
        ...
        ks_df_informations['show_hierarchy'] = show_hierarchy
        hierarchy_move_lines = []
        if show_hierarchy and self.report_name == 'General Ledger':
            hierarchy_data = self.get_general_ledger_hierarchy_data(move_lines[0], start=True)
            if hierarchy_data:
                hierarchy_move_lines = hierarchy_data['undefined']
            if ks_df_informations.get('ks_report_with_lines', False):
                hierarchy_move_lines = self.flatten_data(hierarchy_move_lines)
        ...
        if show_hierarchy:
            for line in hierarchy_move_lines:
                ...
                sheet.merge_range(row_pos, 0, row_pos, 4,
                                  '            ' + line.get('code') + ' - ' + line.get(
                                      'name'),
                                  line_header_left)
                if self.env['ir.config_parameter'].sudo().get_param('ks_enable_ledger_in_bal'):
                    sheet.write_number(row_pos, 5, float(line.get('initial_balance', 0)), line_header)
                    sheet.write_number(row_pos, 6, float(line.get('debit')), line_header)
                    sheet.write_number(row_pos, 7, float(line.get('credit')), line_header)
                    sheet.write_number(row_pos, 8, float(line.get('balance')), line_header)
                else:
                    sheet.write_number(row_pos, 5, float(line.get('debit')), line_header)
                    sheet.write_number(row_pos, 6, float(line.get('credit')), line_header)
                    sheet.write_number(row_pos, 7, float(line.get('balance')), line_header)
                    ...
                    if 'lines' in line:
                        for sub_line in line['lines']:
                            if sub_line['initial_bal']:
                                ...
                                if self.env['ir.config_parameter'].sudo().get_param('ks_enable_ledger_in_bal'):
                                    sheet.write_number(row_pos, 5, float(line.get('initial_balance', 0)),
                                                       line_header_light_initial)
                                    sheet.write_number(row_pos, 6, float(line.get('debit')),
                                                       line_header_light_initial)
                                    sheet.write_number(row_pos, 7, float(line.get('credit')),
                                                       line_header_light_initial)
                                    sheet.write_number(row_pos, 8, float(line.get('balance')),
                                                       line_header_light_initial)
                                else:
                                    sheet.write_number(row_pos, 5, float(line.get('debit')),
                                                       line_header_light_initial)
                                    sheet.write_number(row_pos, 6, float(line.get('credit')),
                                                       line_header_light_initial)
                                    sheet.write_number(row_pos, 7, float(line.get('balance')),
                                                       line_header_light_initial)
                                ...
                                if self.env['ir.config_parameter'].sudo().get_param('ks_enable_ledger_in_bal'):
                                    ...
                                    sheet.write_number(row_pos, 8,
                                                       ...
                                                       float(line.get('initial_balance', 0)),
                                                       line_header_light)
                                else:
                                    sheet.write_number(row_pos, 5,
                                                       float(sub_line.get('debit')), line_header_light)
