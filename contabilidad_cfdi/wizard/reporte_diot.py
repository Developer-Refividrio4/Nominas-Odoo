...
class ReporteDIOTWizard(models.TransientModel):
    ...
    year = fields.Selection([('2025','2025'),('2024','2024'),('2023','2023'),('2022','2022'),('2021','2021')], string="Año", required=1)
    ...
    def action_view_report(self):
            ...
            for iva_pedimentos in iva_pedimentos_ids: 
                ...
                if partner.id not in line_vals:
                    line_vals[partner.id] = {
                                         ...
                                         'rfc' : partner.vat,  #tres
                                         'registro_tributario' : partner.tipo_proveedor == '05' and partner.registro_tributario or '', #cuatro
                                         'partner_id' : partner.tipo_proveedor == '05' and partner.id or '',   #cinco
                                         'nacionalidad' : partner.tipo_proveedor == '05' and partner.pais_diot.nacionalidad or '', #seis
                                         'pais' : partner.tipo_proveedor == '05' and partner.pais_diot.c_pais or ''  #siete
                                         }
                line_vals[partner.id].update({'importacion_16': round(iva_pedimentos.monto_iva / 0.16,2) + line_vals[partner.id].get('importacion_16',0)})
        ...
        try:
            tree_id = self.env.ref('contabilidad_cfdi.reporte_diot_wizard_line_form_view_tree_itadmin').id
        except ValueError:
            tree_id = False
